...
The API Server platform exposes an OpenAI-compatible HTTP API on the gateway. Enable it via environment variables:

```bash
# Required
export API_SERVER_PORT=8080        # Port to listen on (default: 8642)
export API_SERVER_HOST=0.0.0.0     # Bind address (default: 127.0.0.1)
export API_SERVER_KEY="$(openssl rand -hex 32)"  # Required for network-accessible hosts

# Optional
export API_SERVER_CORS_ORIGINS="*"  # CORS origins
export API_SERVER_MODEL_NAME="hermes-agent"  # Model name in responses
```

On Fly.io, set these as secrets:
```bash
fly secrets set -a <app> API_SERVER_PORT=8080 API_SERVER_HOST=0.0.0.0 API_SERVER_KEY="$(openssl rand -hex 32)"
```
...
- `POST /v1/runs` — Structured runs

The API server refuses to start on network-accessible hosts without a real API key (not placeholder). Generate one with `openssl rand -hex 32`.

#### Web UI on Fly.io

The API server serves an API, not a web UI. To add a browser interface at the root URL:

1. Create a simple HTML chat interface and upload it to the persistent volume:
   ```bash
   fly ssh sftp shell -a <app>
   put /tmp/chat.html /root/.hermes/web/index.html
   ```
   Use `fly ssh sftp` for file transfers — `fly ssh console -C` has shell parsing quirks (e.g. `&&` is treated literally, `2>/dev/null` fails with `find`, `bash -c` is blocked by user consent, `ps aux` is not supported in the container).

2. The Hermes source code at `/usr/local/lib/hermes-agent/` lives in the Docker image and is reset on every machine restart. Patches must be reapplied via a startup script on the persistent volume (`/root/.hermes/`). Create `/root/.hermes/startup.sh`:
   ```bash
   #!/usr/bin/env bash
   set -e
   python3 << 'PYEOF'
   ...
   if '_handle_index' not in c:
       c = c.replace('self._app.router.add_post("/v1/chat/completions", self._handle_chat_completions)',
   ...
   ```
   Use `python3 << 'PYEOF'` heredocs instead of `python3 -c` — the `-c` flag is fragile with multi-line quoting and backslash escapes in shell contexts.

3. Set the machine entrypoint directly. Do **not** use `fly.toml` `[processes]` — it fails for apps without a Dockerfile or buildpacks.
   ```bash
   fly machine update <machine_id> -a <app> --entrypoint '/root/.hermes/startup.sh' --yes
   fly machine restart <machine_id> -a <app>
   ```

4. The persistent volume is mounted at `/root/.hermes/` — the Docker image source at `/usr/local/lib/hermes-agent/` is reset on every machine restart.

For the full deployment recipe, error transcripts, and verification steps, see `references/fly-io-webui-deployment.md`.
